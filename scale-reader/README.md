...
HBF-701（Karada Scan）测完后**按键逐项翻页，一屏一项**，所以拍照时请**一屏拍一张**（共 8 项左右）。

| 屏幕读数 | 是否写入苹果健康 | 说明 |
|---|---|---|
| 体重 (kg) | ✅ 写入“体重”(bodyMass) | 直接写 |
| 体脂肪率 (%) | ✅ 写入“体脂肪率”(bodyFatPercentage) | 注意：健康内部用分数表示，代码里已 ÷100 |
| 骨骼肌率 (%) | ⚠️ 换算后写入 | 健康没有“骨骼肌率”，按 **骨骼肌(kg) = 体重 × 骨骼肌率 ÷ 100** 换算后写入“去脂体重 / Lean Body Mass” |
| 皮下脂肪率 (%) | ❌ 仅本地 | 健康无此类型，保留在本 App 的历史/趋势里 |
| 内脏脂肪等级 | ❌ 仅本地 | 健康无此类型 |
| BMI | ✅ 写入“BMI”(bodyMassIndex) | 可单独关闭 |
| 基础代谢 (kcal) | ✅ 写入“静息能量”(basalEnergyBurned) | 默认关闭，避免与手表数据混淆 |
| 身体年龄 (岁) | ❌ 仅本地 | 健康无此类型 |

- 每一项是否写入健康可在 **设置 → 写入苹果健康** 里单独开关。
