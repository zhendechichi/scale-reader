...
HBF-701（Karada Scan）测完后**按键逐屏翻页，一轮约 10 屏**，拍照时请**一屏拍一张、整屏拍全**（含上方人体图示，AI 靠它判断部位）。

| 屏 | 屏幕读数 | 是否写入苹果健康 | 说明 |
|---|---|---|---|
| 1 | 体重 (kg) | ✅ “体重”(bodyMass) | 直接写 |
| 2 | 体脂肪率 (%) | ✅ “体脂肪率”(bodyFatPercentage) | 健康内部用分数表示，代码已 ÷100 |
| 3 | 身体年龄 (岁) | ❌ 仅本地 | 健康无此类型 |
| 4 | BMI | ✅ “BMI”(bodyMassIndex) | 可单独关闭 |
| 5 | 基础代谢 (kcal) | ✅ “静息能量”(basalEnergyBurned) | 默认关闭，避免与手表数据混淆 |
| 6 | 内脏脂肪等级 | ❌ 仅本地 | 健康无此类型 |
| 7 | 全身：皮下脂肪率 / 骨骼肌率 | ⚠️ 骨骼肌率换算后写入 | 骨骼肌(kg) = 体重 × 全身骨骼肌率 ÷ 100 → 写入“去脂体重” |
| 8 | 双臂：皮下脂肪率 / 骨骼肌率 | ❌ 仅本地 | |
| 9 | 躯干：皮下脂肪率 / 骨骼肌率 | ❌ 仅本地 | |
| 10 | 双脚：皮下脂肪率 / 骨骼肌率 | ❌ 仅本地 | |

- 是否写入健康可在 **设置 → 写入苹果健康** 里逐项开关。
- 写入采用逐项独立写入：某项失败只报那项，不影响其余项和本地记录。
- 趋势图支持在全部 14 个指标之间切换查看（下拉菜单）。
